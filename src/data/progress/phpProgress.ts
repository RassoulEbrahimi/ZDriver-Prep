// PHP backend progress client (PHP mode only). Fail-soft blob save/load against
// the standalone PHP/MySQL API. Mirrors the repo.ts contract: every call returns
// a result object and NEVER throws.
//
// The whole progress snapshot is stored as one JSON blob per user (matching the
// backend's one-row-per-user model). localStorage stays the runtime source of
// truth; this is an additive, best-effort cloud mirror — exactly like the
// Firestore repo, but for PHP-authenticated users.
